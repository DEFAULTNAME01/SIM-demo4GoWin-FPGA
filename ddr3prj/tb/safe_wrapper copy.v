...
module ddr3_model_safe_wrapper0 #(
...
module ddr3_model_safe_wrapper #(
    ...
    parameter BA_WIDTH        = 3,
    parameter DQM_WIDTH       = 2,
    parameter ENABLE_DELAY_PS = 10000000
)(
    input  wire                    pll_lock,
    input  wire                    tb_rst_n,
    ...
    // ---------------------------------------------------------
    // 时钟：未使能前给安全静态值；使能后直接透传 DUT 原始时钟
    // 不再对 ck/ck_n 做 sanitize，避免破坏 DDR 时钟边沿关系
    // ---------------------------------------------------------
    assign mem_ck   = model_enable_r ? dut_ck   : 1'b0;
    assign mem_ck_n = model_enable_r ? dut_ck_n : 1'b1;
